...
class get_high_attended_patches(tf.keras.layers.Layer):
    def __init__(self,**kwargs):
        super().__init__(**kwargs)

    def call(self, scores, features):

        topk=tf.cast(0.05*tf.cast(tf.shape(scores)[0],tf.float32), tf.int32)

        values, max_indices = tf.math.top_k(scores, k=topk, sorted=True)

        #values, min_indices = tf.nn.top_k(-scores, k=topk, sorted=True)
